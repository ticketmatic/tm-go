...
//
// Whenever an operation is refused due to rate limiting, you MUST wait in the rate
// limiting queue before retrying the operation or continuing further actions.
//
...
//
// * backoff: Number of milliseconds to wait before requesting a new status update.
// This will change dynamically based on where you are in the queue.
//
// * started: Whether the queue has started (as in: is progressing) or not (as in:
// is waiting until a fixed time before starting).
//
...
//
// * orderid: The ID of the newly created order. Only returned when a create order
// operation was rate limited and subsequently progressed through the queue. This
// can be used to retrieve the new order once progressed through the queue.
//
...
//
// * Use JavaScript on this page to set a timeout for backoff milliseconds and then
// request a new status with the supplied queue ID. You will need to foresee an
// endpoint in your backend that calls the Ticketmatic API to request a new queue
// status and returns it to the client.
//
...
// Go
//
// A rate limited call will return a RateLimitError. This error has a status field
// which contains the rate limiting info.
//
//
...
// PHP
//
// A rate limited call will throw a RateLimitException. This exception has a data
