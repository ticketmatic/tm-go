...
//
// * Manual (2501): delivery of the tickets will happen manually by the seller. This could be via
// a daily or weekly print.
//
// * Automatic after full payment (2502): Ticketmatic will deliver the tickets automatically once
// the outstanding balance for the order reaches 0 (full payment).
//
// * Automatic after confirmation (2503): Ticketmatic will deliver the tickets automatically as
...
// (http://en.wikipedia.org/wiki/JavaScript). To do so, you need to add a usescript and script
...
//      }
//
// Note that the list of sales channel IDs is still required: the script can only restrict this
// set further.
//
...
//
// This script states that the current delivery scenario is only available if the amount of
// tickets in the order is less than 3 and the current sales channel is a web sales channel.
//
...
// * saleschannel
//
// You can use any valid JavaScript syntax (including conditionals and loops). Note that each
// script has a strict time limit.
//
// Help Center
//
